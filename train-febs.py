...
def train(cfg, seed):
    ...
    for epoch in range(cfg.max_epochs):
        ...
        for i, sample in enumerate(train_loader):
            ...
            weight0, weight1 = loss_ce[age<60].mean().clone().detach(), loss_ce[age>=60].mean().clone().detach()
            ...
            weight0, weight1 = loss_dice[age<60].mean().clone().detach(), loss_dice[age>=60].mean().clone().detach()
            weight_min       = min(weight0, weight1)
            weights          = torch.where(age<60, torch.tanh(weight_min/weight0), torch.tanh(weight_min/weight1)).view(-1, 1, 1, 1)
...
if __name__=='__main__':
    ...
    class Config:
        def __init__(self,seed):
            ...
            self.save_path         = f'./results/febs/seed_{seed}/{self.local_time}'
