...
def train(cfg, seed):
    ...
    for epoch in range(cfg.max_epochs):
        ...
        for i, sample in enumerate(train_loader):
            ...
            loss_ce     = loss_ce.mean()+0.5*((loss_ce-loss_ce.mean())**2).mean()
                
            # weight0, weight1 = loss_ce[age<60].mean(), loss_ce[age>=60].mean()
            # ce_mean          = (weight0+weight1)/2
            # loss_rex         = (weight0-ce_mean)**2+(weight1-ce_mean)**2
            # loss_rex         = loss_rex.mean()
            # loss_ce        = loss_ce.mean()+0.5*((loss_ce-loss_ce.mean())**2).mean()
            ...
            # dice_mean        = (weight0+weight1)/2
            # loss_rex         = (weight0-dice_mean)**2+(weight1-dice_mean)**2
            # loss_rex         = loss_rex.mean()
...
if __name__=='__main__':
    ...
    class Config:
        def __init__(self,seed):
            ...
            self.save_path         = f'./results/vrex/seed_{seed}/{self.local_time}'
