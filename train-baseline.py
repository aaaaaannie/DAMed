...
def train(cfg, seed):
    ...
    ## logger
    #logger.add(cfg.save_path+f'/train.log', filter=lambda x: "[train]" in x['message']) 
    #logger.add(cfg.save_path+f'/valid.log',  filter=lambda x: "[valid]" in x['message']) 
    ...
    for epoch in range(cfg.max_epochs):
        ...
        for i, sample in enumerate(train_loader):
            ...
            loss_dice   = (mask*pred).sum(dim=(1,2,3))/((mask+pred).sum(dim=(1,2,3))+1e-6)
            loss_dice   = 1-2*loss_dice.mean()
...
if __name__=='__main__':
    ...
    class Config:
        def __init__(self,seed):
            self.batch_size        = 32
            self.max_epochs        = 30 #original:64
            self.base_lr           = 0.0001
            self.rank              = 4
            self.patch_size        = 16
            self.root              = '../dataset'
            ...
            self.save_path         = f'./results/baseline/seed_{seed}/{self.local_time}'
