...
from utils import ValidData

def test(cfg):
    ## dataset
    test_data                 = ValidData(cfg)
    ...
    with torch.no_grad():
        ...
        for i, sample in enumerate(test_loader):
            image, mask, age = sample['image'].cuda(), sample['mask'].cuda(), sample['age'].item()
            mask        = (mask>0.5).float()
...
if __name__=='__main__':
    class Config:
        def __init__(self):
            ...
            self.root              = '../dataset'
            self.image_size        = 352
            self.snapshot          = 'pretrain/sam_vit_b_01ec64.pth'
            self.lora_snapshot     = './save-vrex-delta/model-56'
