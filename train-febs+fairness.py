...
def train(cfg, seed):
    ...
    baseline_models = glob.glob(cfg.baseline_model_path)
    model.load_lora_parameters(baseline_models[0]) ## 此处加载baseline模型的参数
    ...
    for epoch in range(cfg.max_epochs):
        ...
        for i, sample in enumerate(train_loader):
            ...
            weight0, weight1 = weight0.clone().detach(), weight1.clone().detach()
            weight_min       = min(weight0, weight1)
            weights          = torch.where(age<60, torch.tanh(weight_min/weight0), torch.tanh(weight_min/weight1)).view(-1, 1, 1, 1)
            loss_ce          = -mask*torch.log(pred*weights+1e-6)-(1-mask)*torch.log(1-pred*weights+1e-6)
            ...
            weight_min       = min(weight0, weight1)
            weights          = torch.where(age<60, torch.tanh(weight_min/weight0), torch.tanh(weight_min/weight1)).view(-1, 1, 1, 1)
            loss_dice        = 1-(2*mask*pred*weights).sum(dim=(1,2,3))/((mask+pred*weights).sum(dim=(1,2,3))+1e-6)
...
if __name__=='__main__':
    ...
    class Config:
        def __init__(self,seed):
            ...
            self.save_path         = f'./results/febs-fairness/seed_{seed}/{self.local_time}'
            self.snapshot          = './checkpoints/sam_vit_b_01ec64.pth'
            self.baseline_model_path = f'./results/baseline/seed_{seed}/*/model-20'
