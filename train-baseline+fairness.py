...
def train(cfg, seed):
    ...
    for epoch in range(cfg.max_epochs):
        ...
        for i, sample in enumerate(train_loader):
            ...
            image, mask, age = sample['image'].cuda(), sample['mask'].cuda(), sample['age'].cuda()
            mask             = (mask>0.5).float()
            pred             = model(image, False, cfg.image_size)['masks']
            pred             = torch.sigmoid(pred)
            loss_ce          = -mask*torch.log(pred+1e-6)-(1-mask)*torch.log(1-pred+1e-6)
            weight0, weight1 = loss_ce[age<60].mean(), loss_ce[age>=60].mean()
            loss_ce_delta    = (weight0-weight1).abs()
            loss_ce          = loss_ce.mean()

            loss_dice        = 1-2*(mask*pred).sum(dim=(1,2,3))/((mask+pred).sum(dim=(1,2,3))+1e-6)
            weight0, weight1 = loss_dice[age<60].mean(), loss_dice[age>=60].mean()
            loss_dice_delta  = (weight0-weight1).abs()
            loss_dice        = loss_dice.mean()

            loss             = 0.2*loss_ce+0.8*loss_dice+0.1*loss_dice_delta
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

            if i%10==0:
                logger.info("[train][seed=%d] | iter=%d/%d | epoch=%d/%d | lr=%.4f | loss_ce=%.4f | loss_dice=%.4f | loss_ce_delta=%.4f | loss_dice_delta=%.4f"%(seed,i, len(train_loader), epoch+1, cfg.max_epochs, optimizer.param_groups[0]['lr'], loss_ce.item(), loss_dice.mean().item(), loss_ce_delta.item(), loss_dice_delta.item()))
                #logger.info("[train] | iter=%d/%d | epoch=%d/%d | lr=%.4f | loss_ce=%.4f | loss_dice=%.4f | loss_ce_delta=%.4f | loss_dice_delta=%.4f"%(i, len(train_loader), epoch+1, cfg.max_epochs, optimizer.param_groups[0]['lr'], loss_ce.item(), loss_dice.mean().item(), loss_ce_delta.item(), loss_dice_delta.item()))
...
if __name__=='__main__':
    ...
    class Config:
        def __init__(self,seed):
            ...
            self.save_path         = f'./results/baseline-fairness/seed_{seed}/{self.local_time}'
    ...
    logger.add(os.path.join(cfg.save_path, 'test.log'), filter=lambda x: "[test]" in x['message'])
    train(cfg, seed)
